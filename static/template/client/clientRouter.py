...
# Получение текущей активной заявки клиента
    ...
    try:
        async with AsyncSession(engine_a) as session:
            query = (
                ...
                .join(Address, Requests.addressid == Address.id, isouter=True)
                .join(Company, Address.companyid == Company.id, isouter=True)
                .where(and_(Client.id == client_id,Status.name.notin_(['Выполненная'])
                ))
            )
            result = await session.execute(query)
            rows =  result.fetchall()
...
# Получение общего дохода от клиента
...
# Получение истории статуса запроса, сделанного клиентом
...
# Функция для трансформации дат
def transformation_dates(start_date, end_date):
    start_date_obj = datetime.datetime.strptime(start_date, '%Y-%m-%d').date()
    end_date_obj = datetime.datetime.strptime(end_date, '%Y-%m-%d').date()
    return start_date_obj, end_date_obj

# Получение истории трат клиента в заданный период времени
